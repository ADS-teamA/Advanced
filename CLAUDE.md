...
- Query Expansion: Multi-query generation (+18% recall) with `num_expands` parameter
...
- **TIER 1** (11 tools): Fast (<100ms), basic retrieval - Use first
  - Key tool: **`search`** (unified hybrid search with optional query expansion) ✅ **NEW: Query Expansion**
    ...
    - `num_expands=1`: Balanced mode (~500ms) - original + 1 paraphrase, 2 queries total (+18% recall)
    - `num_expands=2`: Better recall (~800ms) - original + 2 paraphrases, 3 queries total (+25% recall)
...
**Recent Updates:**
- Query Expansion (2025-10-26): Multi-query generation with `num_expands` parameter (+18% recall)
